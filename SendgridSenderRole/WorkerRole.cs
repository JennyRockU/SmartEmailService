...
using SendgridSender;

namespace SendgridWorkerRole
{
    public class WorkerRole : RoleEntryPoint
    {
        private readonly CancellationTokenSource cancellationTokenSource = new CancellationTokenSource();
        private readonly ManualResetEvent runCompleteEvent = new ManualResetEvent(false);
        private readonly string WorkerRoleName = "SendgridRole";
        ...
        private  async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            var sendgridVendor = new Sender();

            while (!cancellationToken.IsCancellationRequested)
            {
                var queueMsg = await queue.GetMessageAsync(cancellationToken);
                if (queueMsg == null) continue;
                var isProcessed = await MessageHandler.ProcessMessage(queueMsg, sendgridVendor, cancellationToken);
